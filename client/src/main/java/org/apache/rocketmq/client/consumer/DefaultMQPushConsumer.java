...
public class DefaultMQPushConsumer extends ClientConfig implements MQPushConsumer {
     ...
     * </li>
     * </ul>
     * CONSUME_FROM_LAST_OFFSET分两种情况：
     *     1）如果磁盘消息未过期且未被删除，则从最小偏移量开始消费。
     *     2）如果磁盘已过期并被删除，则从最大偏移量开始消费
     */
    ...
    /**
     * 并发消息消费时处理队列最大跨度，默认2000；表示如果消息处理队列中偏移量最大的消息与偏移量最小的消息的跨度超过2000，则延时50ms后再拉取消息
    ...
    /**
     * 最大消费重试次数，默认16次。如果消息消费次数超过maxReconsumeTimes还未成功，则将该消息转移到一个失败队列，等待被删除。
